...
async function main() {
  ...
  console.log('⚠️ Note: This script will ONLY insert missing records. It will NOT overwrite existing data or drop tables.');
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isInsideCopyBlock) {
            ...
            if (line.trim() !== '') {
                ...
                tableInserts[currentTable].push(`INSERT INTO public."${currentTable}" ("${currentColumns.join('", "')}") VALUES (${formattedValues.join(', ')}) ON CONFLICT DO NOTHING;\n`);
            }
        }
    }
    ...
    const { stdout, stderr } = await execAsync(`psql "${dbUrl}" -v ON_ERROR_STOP=1 -f "${safeBackupPath}"`);
