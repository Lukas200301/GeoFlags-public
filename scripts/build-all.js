...
function header(text) {
  ...
  log(`${colors.cyan}  ║${colors.bold}${colors.white}${text.padStart(24 + text.length / 2).padEnd(48)}${colors.reset}${colors.cyan}║${colors.reset}`);
  log(`${colors.cyan}  ╚${line}╝${colors.reset}`);
  log('');
}
...
if (allPassed && !runStep('Copy assets', 'npx copyfiles -u 1 "src/**/*.geo.json" dist', './backend')) {
  allPassed = false;
}
log('');
...
if (allPassed) {
  ...
  log('');
  log(`  ${colors.bgGreen}${colors.bold}${colors.white} BUILD SUCCESSFUL ${colors.reset}  ${colors.dim}Total: ${totalDuration}${colors.reset}`);
  log('');
  log(`  ${colors.dim}Run ${colors.reset}${colors.cyan}npm run test:prod${colors.reset}${colors.dim} to start both servers${colors.reset}`);
  log('');
} else {
  const line = '─'.repeat(48);
  log(`  ${colors.dim}${line}${colors.reset}`);
  log('');
  log(`  ${colors.bgRed}${colors.bold}${colors.white} BUILD FAILED ${colors.reset}  ${colors.dim}after ${totalDuration}${colors.reset}`);
  log('');
  process.exit(1);
}
