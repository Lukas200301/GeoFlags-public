  ...
  <div class="max-w-6xl mx-auto">
    ...
    >
      ...
      <p class="text-gray-400 mb-6">
        The country with code "{{ route.params.code }}" could not be found.
      </p>
      <NuxtLink
        to="/learn"
    ...
    <div v-else-if="country" class="space-y-10">
      ...
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-primary-600/5 via-transparent to-secondary-600/5"
        ></div>

        <div class="relative grid grid-cols-1 lg:grid-cols-[3fr,2fr] gap-0">
          <!-- Flag Visual -->
          <div
            class="relative h-80 lg:h-[32rem] bg-gray-800/30 flex items-center justify-center p-6 lg:p-12 overflow-hidden"
          >
            <img
              :src="country.flag"
            ...
            />
            <div
              class="absolute inset-0 bg-gradient-to-t from-gray-900/80 via-gray-900/20 to-transparent pointer-events-none"
            ></div>
          </div>
          ...
          <div class="p-10 lg:p-12 flex flex-col justify-center space-y-6">
            <div>
              <p class="text-sm font-bold text-primary-400 uppercase tracking-widest mb-3">
                {{ country.region }}
              </p>
              <h1
                class="text-5xl lg:text-6xl font-black text-white leading-[1.25] mb-4"
            ...
            <div
              ...
              class="flex items-center space-x-4 p-4 bg-gray-800/40 rounded-xl border border-gray-700/40"
            >
            ...
            <div class="flex flex-wrap gap-2.5">
              <span
                class="px-4 py-2 bg-primary-500/15 border border-primary-600/40 rounded-xl text-sm text-primary-300 font-bold uppercase tracking-wide"
              >
                {{ country.code }}
              </span>
              <span
              ...
              <span
                v-if="country.cioc"
                class="px-4 py-2 bg-secondary-500/15 border border-secondary-600/40 rounded-xl text-sm text-secondary-300 font-bold uppercase tracking-wide"
              >
                {{ country.cioc }}
              </span>
              <span
                v-if="country.idd && country.idd.root"
                class="px-4 py-2 bg-blue-500/15 border border-blue-600/40 rounded-xl text-sm text-blue-300 font-mono font-bold"
              >
                {{ country.idd.root
                }}{{
                  country.idd.suffixes && country.idd.suffixes[0] ? country.idd.suffixes[0] : ''
                }}
              </span>
            </div>
          </div>
      ...
      >
        <!-- Area -->
        <div
          class="group relative bg-gradient-to-br from-blue-900/50 to-blue-950/70 backdrop-blur-xl border-2 border-blue-700/50 rounded-2xl p-8 hover:border-blue-500/70 hover:shadow-2xl hover:shadow-blue-500/25 transition-all duration-300"
        >
          ...
          <p class="text-center text-xs font-bold text-blue-300 uppercase tracking-widest mb-3">
            Total Area
          </p>
          <p
            class="text-center text-5xl font-black bg-gradient-to-br from-white via-blue-50 to-blue-100 bg-clip-text text-transparent"
          >
        ...
        <!-- Population -->
        <div
          v-if="restCountryData?.population"
          class="group relative bg-gradient-to-br from-emerald-900/50 to-emerald-950/70 backdrop-blur-xl border-2 border-emerald-700/50 rounded-2xl p-8 hover:border-emerald-500/70 hover:shadow-2xl hover:shadow-emerald-500/25 transition-all duration-300"
        >
          <div class="flex items-center justify-center mb-6">
          ...
          <p class="text-center text-xs font-bold text-emerald-300 uppercase tracking-widest mb-3">
            Population
          </p>
          <p
            class="text-center text-5xl font-black bg-gradient-to-br from-white via-emerald-50 to-emerald-100 bg-clip-text text-transparent"
          >
            {{ formatPopulation(restCountryData.population) }}
          </p>
        </div>

        <!-- Region/Continent -->
        <div
          class="group relative bg-gradient-to-br from-purple-900/50 to-purple-950/70 backdrop-blur-xl border-2 border-purple-700/50 rounded-2xl p-8 hover:border-purple-500/70 hover:shadow-2xl hover:shadow-purple-500/25 transition-all duration-300"
        >
          <div class="flex items-center justify-center mb-6">
          ...
          <p class="text-center text-xs font-bold text-purple-300 uppercase tracking-widest mb-3">
            Region
          </p>
          <p
            class="text-center text-4xl font-black bg-gradient-to-br from-white via-purple-50 to-purple-100 bg-clip-text text-transparent leading-tight"
          >
            {{ country.region }}
          </p>
          <p
            v-if="country.subregion"
            class="text-center text-sm text-purple-300/80 mt-2 font-semibold"
          >
            {{ country.subregion }}
          </p>
        </div>
      ...
      >
        <!-- Demonym -->
        <div
          v-if="country.demonym"
          class="bg-gray-800/40 backdrop-blur-xl border border-gray-700/50 rounded-xl p-6 hover:border-gray-600/60 transition-all duration-300"
        >
        ...
        >
          ...
          <p v-if="country.unRegionalGroup" class="text-xs text-gray-400 mt-2">
            {{ country.unRegionalGroup }}
          </p>
        </div>
        ...
        >
          ...
          <p class="text-2xl font-bold text-gray-100 capitalize">
            {{ formatCarSide(restCountryData.carSide) }}
          </p>
        </div>

        <!-- Alternative Names -->
        <div
          v-if="country.altSpellings && country.altSpellings.length > 1"
          class="bg-gray-800/40 backdrop-blur-xl border border-gray-700/50 rounded-xl p-6 hover:border-gray-600/60 transition-all duration-300"
        >
          <div class="flex items-center space-x-3 mb-3">
          ...
          <p class="text-sm text-gray-300 font-medium">
            {{ country.altSpellings.slice(0, 3).join(', ') }}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div
          v-if="country.latlng"
          class="bg-gray-800/30 backdrop-blur-xl border border-gray-700/40 rounded-2xl p-7 hover:border-gray-600/50 transition-all duration-300"
        >
        ...
        <!-- Domain Extensions -->
        <div
          v-if="country.tld && country.tld.length > 0"
          class="bg-gray-800/30 backdrop-blur-xl border border-gray-700/40 rounded-2xl p-7 hover:border-gray-600/50 transition-all duration-300"
        >
          <div class="flex items-center space-x-3 mb-5">
      ...
      >
        <!-- Coat of Arms -->
        <div
          v-if="restCountryData?.coatOfArms"
          class="bg-gray-800/30 backdrop-blur-xl border border-gray-700/40 rounded-2xl p-8 hover:border-gray-600/50 transition-all duration-300"
        >
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            ...
            >
              ...
              <div class="absolute inset-0">
                ...
                <!-- Gradient Overlay -->
                <div
                  class="absolute inset-0 bg-gradient-to-t from-gray-900/95 via-gray-900/60 to-transparent"
                ></div>
              </div>
              ...
              <div class="relative h-full flex flex-col justify-end p-3.5">
                <p
                  class="text-sm font-bold text-white group-hover:text-primary-400 transition-colors drop-shadow-lg leading-tight"
                >
