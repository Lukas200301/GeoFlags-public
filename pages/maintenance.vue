<template>
  <div class="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="fixed inset-0 z-0 pointer-events-none overflow-hidden">
      <div
        class="absolute top-1/4 left-1/4 w-96 h-96 bg-amber-500/10 rounded-full filter blur-3xl animate-pulse"
        style="animation-duration: 4s"
      ></div>
      <div
        class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-orange-500/10 rounded-full filter blur-3xl animate-pulse"
    ...
    <div class="relative z-10 max-w-lg w-full text-center">
      ...
      <!-- Main Card -->
      <div
        class="bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-amber-500/20 shadow-2xl p-8 md:p-12"
      >
        <!-- Maintenance Icon -->
        <div
          class="w-20 h-20 mx-auto mb-6 rounded-full bg-amber-500/20 flex items-center justify-center"
        >
          <Icon
            name="mdi:wrench"
            class="w-10 h-10 text-amber-400 animate-bounce"
            style="animation-duration: 2s"
          />
        </div>

        <!-- Title -->
        <h1 class="text-3xl md:text-4xl font-bold text-gray-100 mb-4">Under Maintenance</h1>
        ...
        <!-- Status indicator -->
        <div
          class="flex items-center justify-center gap-3 mb-8 px-4 py-3 rounded-xl bg-amber-500/10 border border-amber-500/20"
        >
      ...
      <!-- Footer -->
      <p class="mt-6 text-gray-500 text-sm">Thank you for your patience 🌍</p>
    </div>
  </div>
</template>
