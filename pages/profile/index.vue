<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div v-else-if="profile" class="space-y-6">
      <!-- Ban/Suspend Warning -->
      <div v-if="profile.status === 'BANNED'" class="glass-card bg-red-900/20 border border-red-500/50 p-6">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-red-300 mb-2">{{ profile.banReason || 'Your account has been banned by an administrator.' }}</p>
      ...
      <div v-else-if="profile.status === 'SUSPENDED'" class="glass-card bg-yellow-900/20 border border-yellow-500/50 p-6">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-yellow-300 mb-2">{{ profile.banReason || 'Your account has been suspended by an administrator.' }}</p>
        ...
        <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div class="relative">
            <div class="w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-5xl font-bold text-white shadow-2xl overflow-hidden">
              <img v-if="profile.avatarUrl" :src="getAvatarUrl(profile.avatarUrl)" alt="Avatar" class="w-full h-full object-cover">
            ...
            >
              <Icon
                :name="profile.status === 'ACTIVE' ? 'mdi:check' : profile.status === 'BANNED' ? 'mdi:close' : 'mdi:pause'"
      ...
      <div class="grid grid-cols-1 gap-6">
        ...
        <NuxtLink
          to="/profile/stats"
          class="glass-card p-6 hover-lift group cursor-pointer"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <div class="w-14 h-14 rounded-xl bg-cyan-500/20 flex items-center justify-center group-hover:bg-cyan-500/30 transition-colors">
            ...
            <Icon name="mdi:chevron-right" class="w-6 h-6 text-gray-400 group-hover:text-cyan-400 group-hover:translate-x-1 transition-all" />
          </div>
        </NuxtLink>
      </div>
    </div>
